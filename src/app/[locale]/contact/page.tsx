...
export default async function ContactPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <article className="space-y-5">
      ...
      <form className="grid gap-4 rounded-2xl border border-slate-200 bg-white p-6 md:grid-cols-2">
        ...
        <button type="button" className="rounded-full bg-stoneSky px-6 py-2 text-white md:col-span-2 md:w-fit">Submit request</button>
      </form>
      <p className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-amber-900">{d.contactPrototype}</p>
    </article>
  );
}
